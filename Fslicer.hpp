...
struct QuantKeyTriHash
{
	...
	{
		auto h = [](int32_t v) -> size_t { return static_cast<uint32_t>(v) * 2654435761u; };
...
inline void emitEdgesFromTriangles(al::Mesh& edges, const al::Mesh& tris, const al::Color& c)
{
	...
	{
		edges.color(c); edges.vertex(v[i + 0]);
		edges.color(c); edges.vertex(v[i + 1]);

		edges.color(c); edges.vertex(v[i + 1]);
		edges.color(c); edges.vertex(v[i + 2]);
